...
contract IDVerifierRegistrar is IDVerifierBase {
        ...
        returns (bool)
    {
        ...
        require(memberData[_authID].authority == 0); // dev: member ID
    ...
    /**
        ...
        @dev
            Restricting an authority will also restrict every member that
    ...
    */
        ...
        returns (bool)
    {
        ...
        require(memberData[_id].authority == 0); // dev: member ID
        if (!_checkMultiSig(false)) return false;
        bytes32 _authID = idMap[msg.sender].id;
        _setMember(_authID, _id, _country, _region, _rating, _expires);
        emit NewMember(_id, _country, _region, _rating, _expires, _authID);
    ...
    function updateMember(
        ...
        returns (bool)
    {
        _authorityCheck(memberData[_id].country);
        if (!_checkMultiSig(false)) return false;
        bytes32 _authID = idMap[msg.sender].id;
        _setMember(_authID, _id, 0, _region, _rating, _expires);
        emit UpdatedMember(_id, _region, _rating, _expires, _authID);
        return true;
    }
        ...
        returns (bool)
    {
        ...
        emit MemberRestriction(_id, _restricted, idMap[msg.sender].id);
        return true;
    }

    /**
        @notice Modify an member's registering authority
        @dev
            This function is used by the owner to reassign members to an
            unrestricted authority if their original authority was restricted.
        @param _authID Authority ID
        @param _id Member ID
        @return bool success
     */
    function setMemberAuthority(
        bytes32 _authID,
        bytes32[] _id
    )
        external
        returns (bool)
    {
        ...
        for (uint256 i; i < _id.length; i++) {
            require(memberData[_id[i]].country != 0); // dev: unknown ID
            Member storage inv = memberData[_id[i]];
