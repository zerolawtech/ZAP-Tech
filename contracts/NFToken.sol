...
contract NFToken {
        ...
        returns (bool)
    {
        ...
        balances[msg.sender].balance -= _stop - _start;
        balances[_to].balance += _stop - _start;
        _transferRange(_pointer, msg.sender, _to, _start, _stop);
        return true;
    }
    ...
    /**
        ...
        @param _bigVal Amount to transfer
     */
    function _transfer(address _from, address _to, uint256 _bigVal) internal {
        require(_bigVal != 0);
        uint48 _value = uint48(_bigVal);
        require (_value == _bigVal);
        require (_from != _to);
        Balance storage b = balances[_from];
        require(_value <= b.balance);
        b.balance -= _value;
        balances[_to].balance += _value;
        emit Transfer(_from, _to, _value);

        if (_value < b.balance && b.ranges.length > 2) {
            /* check for possible range merges */
            uint256 _count;
            uint48[] memory _range = new uint48[](b.ranges.length);
            for (uint256 i; i < b.ranges.length; i++) {
                if (!_checkTime(b.ranges[i])) continue;
                if (rangeMap[b.ranges[i]-1].owner != _to) continue;
                _range[_count] = b.ranges[i];
                _count++;
            }
            if (_count > 0) {
                _value = _transferFromRange(_range, _count, _from, _to, _value);
                if (_value == 0) return;
            }
        }
        if (_transferFromRange(b.ranges, b.ranges.length, _from, _to, _value) > 0) {
            revert();
        }
    }
    ...
    /**
        ...
        @param _length Length of range array
        @param _from Sender address
        @param _to Recipient address
        ...
        returns (uint48)
    {
        for (uint256 i; i < _length; i++) {
            ...
            if (!_checkTime(_range[i])) continue;
            ...
            _transferRange(_start, _from, _to, _start, _stop);
            if (_value == 0) break;
        }
        return _value;
    }
