...
def _check(nft, cust, upper, *expected_ranges):
    assert nft.balanceOf(cust) == sum((x[1] - x[0]) for i in expected_ranges for x in i[1] if x)
    for num, expected in enumerate(expected_ranges, start=1):
        ...
        _compare_ranges(nft, upper, held, num, "0x0000000000000000000000000000000000000000")
        ranges = nft.custodianRangesOf(address, cust)
        assert set(ranges) == set(custodied)
        assert nft.custodianBalanceOf(address, cust) == sum((i[1] - i[0]) for i in custodied)
...
def _compare_ranges(nft, upper, ranges, num, custaddress):
    ...
    for start, stop in ranges:
        if stop - start == 1:
            assert nft.getRange(start) == (address, start, stop, 0, "0x0000", custaddress)
            continue
