...
def test_inside_one(check_ranges, nft):
    """inside, one nft"""
...
def test_one_left_merge(check_ranges, nft):
...
def test_one_right_merge(check_ranges, nft):
...
def test_one_right_merge_abs(check_ranges, nft):
    """one nft, touch right, merge absolute"""
    nft.transferRange(accounts[3], 20000, 20001, {"from": accounts[2]})
    check_ranges([(1, 10001)], [(10001, 20000)], [(20000, 30001)], [])


def test_create_one_start(check_ranges, nft):
...
def test_create_one_start_abs(check_ranges, nft):
    """create one nft range at start, absolute"""
    nft.transferRange(accounts[4], 2, 1000, {"from": accounts[1]})
    check_ranges(
        [(1, 2), (1000, 10001)], [(10001, 20001)], [(20001, 30001)], [(2, 1000)]
    )


def test_create_one_end(check_ranges, nft):
...
def test_create_one_end_abs(check_ranges, nft):
