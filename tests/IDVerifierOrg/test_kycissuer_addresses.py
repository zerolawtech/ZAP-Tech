...
def test_add_addresses_known_address(ikyc):
    ...
    ikyc.addMember(
        "0x123456", 1, 1, 1, 9999999999, accounts[5:7], {"from": accounts[0]}
    )
...
def test_add_address_repeat(ikyc):
    ...
    with pytest.reverts("dev: known address"):
        ikyc.registerAddresses(
            id_, (accounts[5], accounts[6], accounts[5]), {"from": accounts[0]}
        )
...
def test_owner_add_member_addresses(ikyc):
    """owner - add member addresses"""
    ikyc.addMember(
        "0x123456", 1, 1, 1, 9999999999, (accounts[5],), {"from": accounts[0]}
    )
    ikyc.registerAddresses(
        "0x123456", (accounts[6], accounts[7]), {"from": accounts[0]}
    )
