...
def test_add_addresses_known_address(kyc, owner_id, auth_id):
    ...
    with pytest.reverts("dev: known address"):
        kyc.registerAddresses(
            owner_id, (accounts[1], accounts[5]), {"from": accounts[0]}
        )
    with pytest.reverts("dev: known address"):
        kyc.registerAddresses(
            owner_id, (accounts[3], accounts[5]), {"from": accounts[0]}
        )
    with pytest.reverts("dev: known address"):
    ...
    with pytest.reverts("dev: known address"):
        kyc.registerAddresses(
            kyc.getID(accounts[6]), (accounts[3],), {"from": accounts[0]}
        )
...
def test_add_address_repeat(kyc, owner_id):
    ...
    with pytest.reverts("dev: known address"):
        kyc.registerAddresses(
            owner_id, (accounts[1], accounts[2], accounts[1]), {"from": accounts[0]}
        )
...
def test_authority_add_authority_addresses(kyc, owner_id, auth_id):
    ...
    with pytest.reverts("dev: not owner"):
        kyc.registerAddresses(
            owner_id, (accounts[1], accounts[2]), {"from": accounts[-1]}
        )
    with pytest.reverts("dev: not owner"):
        kyc.registerAddresses(
            auth_id, (accounts[1], accounts[2]), {"from": accounts[-1]}
        )
...
def test_authority_restrict_authority_addresses(kyc, owner_id, auth_id):
    ...
    with pytest.reverts("dev: not owner"):
        kyc.restrictAddresses(
            auth_id, (accounts[3], accounts[4]), {"from": accounts[-1]}
        )
...
def test_owner_add_member_addresses(kyc):
    ...
    kyc.addMember(
        "0x123456", 1, 1, 1, 9999999999, (accounts[1],), {"from": accounts[0]}
    )
...
def test_authority_add_member_addresses(kyc):
    ...
    kyc.addMember(
        "0x123456", 1, 1, 1, 9999999999, (accounts[1],), {"from": accounts[-1]}
    )
    kyc.registerAddresses(
        "0x123456", (accounts[2], accounts[3]), {"from": accounts[-1]}
    )
    assert kyc.getID(accounts[2]) == "0x123456"
    assert kyc.getID(accounts[3]) == "0x123456"
...
def test_authority_add_addresses_not_permitted_country(kyc, auth_id):
    ...
    kyc.addMember(
        "0x123456", 2, 1, 1, 9999999999, (accounts[1],), {"from": accounts[0]}
    )
