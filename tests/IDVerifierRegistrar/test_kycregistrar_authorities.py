...
def test_add_exists_as_member(kyc, rpc):
    ...
    rpc.revert()
    kyc.addMember(
        id_, 1, 1, 1, 9999999999, (accounts[1], accounts[2]), {"from": accounts[0]}
    )
...
def test_add_repeat_address(kyc):
    ...
    with pytest.reverts("dev: known address"):
        kyc.addAuthority(
            (accounts[1], accounts[1]), (1, 2, 3), 2, {"from": accounts[0]}
        )
...
def test_restricted_owner(kyc):
    ...
    with pytest.reverts("dev: owner"):
        kyc.setAuthorityRestriction(
            kyc.getAuthorityID(accounts[0]), False, {"from": accounts[0]}
        )
