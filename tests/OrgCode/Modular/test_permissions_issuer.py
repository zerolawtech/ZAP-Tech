...
def test_share_modifyAuthorizedSupply(org, share):
    ...
    _check_permission(
        org, share, "0xc39f42ed", share.modifyAuthorizedSupply.encode_input("10 ether")
    )
...
def test_share_mint(org, share):
    """share mint"""
    _check_permission(
        org, share, "0x40c10f19", share.mint.encode_input(accounts[3], 10000)
    )
...
def test_share_burn(org, share):
    ...
    share.transfer(accounts[2], 5000, {"from": accounts[0]})
    _check_permission(
        org, share, "0x9dc29fac", share.burn.encode_input(accounts[2], 3000)
    )
    assert share.balanceOf(accounts[2]) == 2000
...
def test_nft_mint(org, nft):
    """nft mint"""
    _check_permission(
        org, nft, "0x15077ec8", nft.mint.encode_input(accounts[2], 10000, 0, "0xff11")
    )
...
def test_nft_burn(org, nft):
    """nft burn"""
    _check_permission(org, nft, "0x9a0d378b", nft.burn.encode_input(1337, 31337))
...
def test_nft_modifyRange(org, nft):
    """nft modifyRange"""
    _check_permission(
        org, nft, "0x712a516a", nft.modifyRange.encode_input(1, 0, "0xabcd")
    )
...
def test_nft_modifyRanges(org, nft):
    """nft modifyRanges"""
    _check_permission(
        org, nft, "0x786500aa", nft.modifyRanges.encode_input(100, 200, 0, "0x1111")
    )
