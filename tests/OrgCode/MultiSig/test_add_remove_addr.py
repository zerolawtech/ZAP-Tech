...
def test_add_remove_owner(org, ownerid):
    ...
    org.addAuthorityAddresses(
        ownerid, (accounts[-10], accounts[-9], accounts[-4]), {"from": accounts[0]}
    )
...
def test_add_remove_auth(org, id1, id2):
    ...
    org.addAuthorityAddresses(
        id1, (accounts[-10], accounts[-9], accounts[-5]), {"from": accounts[0]}
    )
...
def test_add_known(org, id1):
    ...
    with pytest.reverts("dev: known address"):
        org.addAuthorityAddresses(
            id1, (accounts[-6], accounts[-5], accounts[-6]), {"from": accounts[0]}
        )
...
def test_add_other(set_countries, org, kyc, share, id1):
    ...
    kyc.addMember(
        b"member1", 1, "0x000001", 1, 9999999999, (accounts[1],), {"from": accounts[0]}
    )
...
def test_remove_repeat(org, id1):
    ...
    with pytest.reverts("dev: already restricted"):
        org.removeAuthorityAddresses(
            id1, (accounts[-10], accounts[-9], accounts[-10]), {"from": accounts[0]}
        )
...
def test_authority_remove_self(org, id1, id2):
    ...
    org.removeAuthorityAddresses(
        id2, (accounts[-8], accounts[-1]), {"from": accounts[-1]}
    )
