...
def test_moved_before(cp, share, cust, cptime):
    ...
    for i in range(1, 6):
        cust.transferInternal(
            share, accounts[i], accounts[0], 1000, {"from": accounts[0]}
        )
    rpc.sleep(110)
    for i in range(1, 6):
...
def test_moved_before_after(cp, share, cust, cptime):
    ...
    for i in range(1, 6):
        assert cp.custodianBalanceAt(share, accounts[i], cust, cptime) == (i - 1) * 1000
        cust.transferInternal(
            share, accounts[0], accounts[i], 1000, {"from": accounts[0]}
        )
        assert cp.custodianBalanceAt(share, accounts[i], cust, cptime) == (i - 1) * 1000
...
def test_two_moved_before(cp, share, cust, cptime):
    ...
    for i in range(1, 6):
        assert cp.custodianBalanceAt(share, accounts[i], cust, cptime) == (i - 1) * 1000
        assert (
            cp.custodianBalanceAt(share, accounts[i], cust, cptime + 100)
            == (i - 1) * 1000
        )
...
def test_three_before(cp, share, cust, cptime):
    ...
    for i in range(1, 6):
        ...
        assert (
            cp.custodianBalanceAt(share, accounts[i], cust, cptime + 200)
            == (i - 1) * 1000
        )
